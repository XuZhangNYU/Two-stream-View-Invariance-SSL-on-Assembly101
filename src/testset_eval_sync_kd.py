...
class Trainer:
    def __init__(self):
        ...
        # self.model = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=2048, nhead=32), num_layers=6) 
        self.model = Transformer(d_model=2048, nhead=16, num_encoder_layers=10)  
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % 10 == 0:
                torch.save({
                    ...
                    'model_state_dict': self.model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),  # Optional, if you want to resume training
                }, save_dir + "/c2f_epoch-" + str(epoch + 1) + ".model")
    ...
    def predict(self, model_dir, results_dir, eval_loader, postprocessor_eval):
        ...
        with torch.no_grad():
            ...
            transformer_weight_name = '/final_transformer_epoch-200-v3.model'
            c2f_weight_name = '/c2f_baseline_final_epoch-400-v3.model'

            self.model.load_state_dict(torch.load(model_dir + transformer_weight_name, map_location=device))
            self.C2F_model.load_state_dict(torch.load(model_dir + c2f_weight_name, map_location=device)['model_state_dict'])
            ...
            print('used transformer weight at, ', transformer_weight_name, 'head: ', c2f_weight_name)
            print('VIEWS', VIEWS)
...
# ！！！！！！！！！！！！！！！！ change paths
vid_list_path = "/home/xz4863/scratch/o_c2f/coarse-annotations/coarse_splits/" 
gt_path = "/home/xz4863/scratch/o_c2f/coarse-annotations/coarse_labels/" 
mapping_file = "/home/xz4863/scratch/o_c2f/actions.csv" 
...
if args.action == "predict":
    ...
    if not os.path.exists(os.path.join(results_dir, 'prediction_naivessl{}'.format('_aug' if int(args.test_aug) else ''))):
        os.makedirs(os.path.join(results_dir, 'prediction_naivessl{}'.format('_aug' if int(args.test_aug) else '')))
    trainer.predict(model_dir, os.path.join(results_dir, 'prediction_naivessl{}'.format('_aug' if int(args.test_aug) else '')),
