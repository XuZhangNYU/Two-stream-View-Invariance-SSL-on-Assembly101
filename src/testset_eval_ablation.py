...
class Trainer:
    ...
    def predict(self, model_dir, results_dir, eval_loader, postprocessor_eval):
        ...
        with torch.no_grad():
            ...
            transformer_weight_name = '/ssl_fallb_epoch-80-v3.model'
            # c2f_weight_name = '/c2f_epoch-best-batch64-large_step-chunk20-maxfram1200-v3.model'
            c2f_weight_name = '/final_head_fallb_walked80_epoch-400-v3.model'
            ...
            for i, item in enumerate(eval_loader):
                ...
                with torch.no_grad():
                    transformed_samples = self.model(samples)
                    # transformed_samples = samples
                residual_out = transformed_samples + samples
                residual_out = residual_out.permute(0, 2, 1)
                # residual_out = residual_out.to(device)

                outputs_list = self.C2F_model(residual_out)
                outputs_ensemble = get_c2f_ensemble_output(outputs_list, config.ensem_weights)
...
if args.action == "predict":
    ...
    if not os.path.exists(os.path.join(results_dir, 'prediction_fallb{}'.format('_aug' if int(args.test_aug) else ''))):
        os.makedirs(os.path.join(results_dir, 'prediction_fallb{}'.format('_aug' if int(args.test_aug) else '')))
    trainer.predict(model_dir, os.path.join(results_dir, 'prediction_fallb{}'.format('_aug' if int(args.test_aug) else '')),
