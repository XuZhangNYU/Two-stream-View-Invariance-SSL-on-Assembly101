...
class Trainer:
    ...
    def predict(self, model_dir, results_dir, eval_loader, postprocessor_eval):
        ...
        with torch.no_grad():
            ...
            transformer_weight_name = '/ssl_vic_epoch-250-v3.model'
...
if args.action == "predict":
    ...
    if not os.path.exists(os.path.join(results_dir, 'prediction_tri_vic{}'.format('_aug' if int(args.test_aug) else ''))):
        os.makedirs(os.path.join(results_dir, 'prediction_tri_vic{}'.format('_aug' if int(args.test_aug) else '')))
    trainer.predict(model_dir, os.path.join(results_dir, 'prediction_tri_vic{}'.format('_aug' if int(args.test_aug) else '')),
