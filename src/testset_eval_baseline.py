...
class Trainer:
    ...
    def predict(self, model_dir, results_dir, eval_loader, postprocessor_eval):
        ...
        with torch.no_grad():
            self.model.to(device)
            # weight_name = '/c2f_epoch-100-batch64-large_step-chunk20-maxfram1200-v3.model'
            weight_name = '/c2f_baseline_final_epoch-400-v3.model'
            self.model.load_state_dict(torch.load(model_dir + weight_name, map_location=device)['model_state_dict'])
            ...
            print('used weight at, ', weight_name)
            print('VIEWS', VIEWS)
...
if args.action == "predict":
    ...
    if not os.path.exists(os.path.join(results_dir, 'prediction_tri{}'.format('_aug' if int(args.test_aug) else ''))):
        os.makedirs(os.path.join(results_dir, 'prediction_tri{}'.format('_aug' if int(args.test_aug) else '')))
    trainer.predict(model_dir, os.path.join(results_dir, 'prediction_tri{}'.format('_aug' if int(args.test_aug) else '')),
