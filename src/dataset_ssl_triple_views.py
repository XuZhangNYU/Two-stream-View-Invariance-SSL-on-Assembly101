...
def collate_fn_override(data):
    ...
    data_arr, data_arr_teacher, data_arr_3rd, count, labels, clip_length, start, video_id, labels_present_arr, aug_chunk_size = zip(*data) #

    return torch.stack(data_arr), torch.stack(data_arr_teacher),  torch.stack(data_arr_3rd), torch.tensor(count), torch.stack(labels), torch.tensor(clip_length), \
...
class AugmentDataset(torch.utils.data.Dataset):
    def __init__(self, args, fold, fold_file_name, actions_dict, zoom_crop=(0.5, 2), smallest_cut=1.0):
        ...
        self.features_path_teacher = args.teacher_features_path #teacher feature path
        self.features_path_3rd = args.features_path_3rd #3rd feature path
        ...
        self.env_3rd = {view: lmdb.open(self.features_path_3rd, readonly=True, lock=False) for view in args.VIEWS} #

        with open('/home/xz4863/scratch/o_c2f/data/statistic_input_v3v4e4.pkl', 'rb') as f:
    ...
    # this returns data_arr: basically have all the labels of corresponding video.
    # data_arr's id is just videos' id. Each video can corresponds to multiple items in data_arr
    # because the video might be too long, and each item in the data_arr will hold the max_frames_per_video
    # of frames for that video. If the videos is not divisible by max_frames_per_video, then of course there is a tail
    # that corresponds to the length (vid_len % max_frames_per_video). This length is also the len of the label for each item.
    # each element in the label is the label of the corresponding frame. Each item in data_arr also has the view info.
    # the view entry is the same one as in self.VIEWS array.
    def make_data_set(self, fold_file_name):
        ...
        if self.fold == 'train':
            if self.split == 'train_val':
                ...
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                files = ['FINAL_USE_Train_Pair_Triple.txt']
        elif self.fold == 'val':
            files = ['val_coarse_assembly.txt', 'val_coarse_disassembly.txt']
            # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            files = ['FINAL_USE_Val_vic.txt']
        ...
        for i, video_id in enumerate(data):
            ...
            filename = os.path.join(self.ground_truth_files_dir, video_id.strip() + ".txt")
            # if counter < 10:
            #     print(i)
            #     print(video_id)
            #     counter += 1
            ...
            max_vid_frame = max(indexs)

            span = [min(indexs), max_vid_frame - (max_vid_frame %1000)]  # [start end)

            recog_content = recog_content[:(span[1] - span[0])]
            ...
            for view in self.VIEWS:
                ...
                key_id = video_id.split(type_action)[1][1:].strip()
                # if counter < 10:

                #     print(self.statistic.keys())
                if view not in self.statistic[key_id]:
                    
                    print('got continue since not found in statics')
                    print('key is',key_id)
                    print()
                    print('all keys are',self.statistic.keys())
                    continue
    ...
    # according to index, find a item in data_arr. The view is of course based on the view of data_arr's item
    def getitem(self, index):  # Try to use this for debugging purpose
        ...
        with self.env[view].begin() as e, self.env_teacher[view].begin() as f, self.env_3rd[view].begin() as third: #  
            for i in range(st_frame, end_frame):
                key_student =  ele_dict['video_id'] + self.frames_format.format('HMC_21179183_mono10bit', 'HMC_21179183_mono10bit', i)
                # if key_student == 'nusar-2021_action_both_9074-a03_9074_user_id_2021-02-11_151600/HMC_21179183_mono10bit/HMC_21179183_mono10bit_0000005069.jpg':
                #     key_student = 'nusar-2021_action_both_9074-a03_9074_user_id_2021-02-11_151600/HMC_21179183_mono10bit/HMC_21179183_mono10bit_0000005068.jpg'
                
                key = ele_dict['video_id'] + self.frames_format.format(view, view, i)
                
                key_3rd = ele_dict['video_id'] + self.frames_format.format('C10119_rgb', 'C10119_rgb', i)

                data = e.get(key_student.strip().encode('utf-8'))
                data_teacher = f.get(key.strip().encode('utf-8')) #
                data_3rd = third.get(key_3rd.strip().encode('utf-8')) #

                if data is None:
                    print('&', end='.')
                    key_student =  ele_dict['video_id'] + self.frames_format.format('HMC_84358933_mono10bit', 'HMC_84358933_mono10bit', i)
                    data = e.get(key_student.strip().encode('utf-8'))

                if data is None:
                    print('no student available data.')
                    print(key_student.strip().encode('utf-8'))
                    print(key_student)
                    data = e.get(key_student.strip().encode('utf-8'))
                    print(data)
                    exit(2)
                ...
                assert data_teacher.shape[0] == 2048 #

                data_3rd = np.frombuffer(data_3rd, 'float32') #
                assert data_3rd.shape[0] == 2048 #


                elements.append(data)
                elements_teacher.append(data_teacher) #
                elements_3rd.append(data_3rd) #
                
        elements = np.array(elements).T
        elements_teacher = np.array(elements_teacher).T #
        elements_3rd = np.array(elements_3rd).T
        ...
        # print(self.max_frames_per_video)
        ...
        data_arr_3rd = torch.zeros((self.max_frames_per_video, self.feature_size)) #
        ...
        for i in range(aug_start_frame, aug_end_frame, aug_chunk_size):
            ...
            key_3rd = elements_3rd[:, i - aug_start_frame: end - aug_start_frame] #
            ...
            data_arr_3rd[count, :] = torch.tensor(np.max(key_3rd, axis=-1), dtype=torch.float32) #
        ...
        return data_arr, data_arr_teacher, data_arr_3rd, count, label_arr, ele_dict['tot_frames'], st_frame, vid_type + '_' + ele_dict[
