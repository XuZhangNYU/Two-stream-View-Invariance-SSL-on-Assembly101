...
class Trainer:
    def __init__(self):
        ...
        # self.reseter = nn.TransformerEncoder(nn.TransformerEncoderLayer(d_model=2048, nhead=32), num_layers=36) 
        self.ce = nn.CrossEntropyLoss(ignore_index=-100)
        self.mse = nn.MSELoss(reduction='none')

        model_weight_name = '/c2f_baseline_triple_final_epoch-240-v3.model'
        # c2f_weight_name = '/c2f_epoch-best-batch64-large_step-chunk20-maxfram1200-v3.model'

        self.model.load_state_dict(torch.load(model_dir + model_weight_name, map_location=device)['model_state_dict'])
        print('successfully load model weights from checkpoint', model_weight_name)
    ...
    def train(self, save_dir, num_epochs):
        if (args.use_check_point != "None"):
            try:
                ...
                # }, save_dir + "/bold_epoch-latest" + ".model")
        ...
        for epoch in range(num_epochs):
            ...
            for i, item in enumerate(train_loader):
                ...
                outputs_list = self.model(samples.permute(0, 2, 1))
            ...
            if (epoch + 1) % 20 == 0:
                torch.save({
                    ...
                    'model_state_dict': self.model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),  # Optional, if you want to resume training
                }, save_dir + "/c2f_baseline_triple_final_epoch-" + str(epoch + 241) + ".model")
            ...
            if test_score > best_score:
                ...
                torch.save({
                    ...
                    'model_state_dict': self.model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),  # Optional, if you want to resume training
                }, save_dir + "/c2f_baseline_triple_final_epoch-best" + ".model")
...
if args.dataset == "assembly":
    ...
    config.chunk_size = 20
    config.max_frames_per_video = 1200
    config.learning_rate = 1e-3 * 0.02
